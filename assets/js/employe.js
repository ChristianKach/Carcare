 $(document).ready(function() {
    ...
    $('.btn_delete').click(function(e) {
        e.preventDefault();
        var employe_id = $(this).attr('data-id');
        var re = confirm('Êtes vous sûr de vouloir supprimer ce employé?');
        if(re) {
          window.location.href = site_url + 'employe/delete/' + employe_id;
        }
    });
    ...
    $('#btn_delete_many').click(function(e) {
        ...
        if(items_checked.length > 1)
             re = confirm('Êtes vous sûr de vouloir supprimer les employés sélectionnés?');
        else
            re = confirm('Êtes vous sûr de vouloir supprimer ce employé sélectionné?');

        if(re) {
            $(this).html('Suppression en cours...');
            var form = document.createElement("form");
                form.setAttribute("method", 'post');
                form.setAttribute("action", site_url + 'employe/delete-many');
