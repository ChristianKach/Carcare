 $(document).ready(function() {
    ...
    $('.btn_delete').click(function(e) {
        ...
        var re = confirm('Êtes vous sûr de vouloir supprimer cet coefficient de garage?');
        if(re) {
          window.location.href = site_url + 'garage/delete/' + garage_id;
        }
    });
    ...
    $('#btn_delete_many').click(function(e) {
        ...
        if(items_checked.length > 1)
             re = confirm('Êtes vous sûr de vouloir supprimer les garages sélectionnées?');
        else
            re = confirm('Êtes vous sûr de vouloir supprimer cette garage sélectionnée?');

        if(re) {
            $(this).html('Suppression en cours...');
            var form = document.createElement("form");
                form.setAttribute("method", 'post');
                form.setAttribute("action", site_url + 'garage/delete-many');
