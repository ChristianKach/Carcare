 $(document).ready(function() {
    ...
    $('.btn_delete').click(function(e) {
        ...
        var re = confirm('Êtes vous sûr de vouloir supprimer ce service?');
        if(re) {
          window.location.href = site_url + 'service/delete/' + service_id;
        }
    });
    ...
    $('#btn_delete_many').click(function(e) {
        ...
        if(items_checked.length > 1)
             re = confirm('Êtes vous sûr de vouloir supprimer les services sélectionnés?');
        else
            re = confirm('Êtes vous sûr de vouloir supprimer ce service sélectionné?');

        if(re) {
            $(this).html('Suppression en cours...');
            var form = document.createElement("form");
                form.setAttribute("method", 'post');
                form.setAttribute("action", site_url + 'service/delete-many');
